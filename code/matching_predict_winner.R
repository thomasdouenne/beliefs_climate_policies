...
# Compute matching: TODO: choose best specification
out.nnd <- NND.hotdeck(
  data.rec = df_receiver_bdf, data.don = df_donor_enl,
  match.vars = c("accommodation_size", "hh_income", "consumption_units", "age_18_24", "age_25_34", "age_35_49", "age_50_64"),
...
sum(fused.nnd.m$winner * fused.nnd.m$predicted_winner_matching) / sum(fused.nnd.m$winner)
sum((fused.nnd.m$winner == 0) * (fused.nnd.m$predicted_winner_matching == 0)) / sum((fused.nnd.m$winner == 0))
