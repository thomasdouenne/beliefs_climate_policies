...
%DIF < %\usepackage{mathptmx} % Times New Roman AER
%DIF -------
\documentclass[12pt]{article} % 11 pt %DIF > 
\usepackage{mathptmx} % Times New Roman AER %DIF > 
%DIF -------
...
%DIF 36-38c39-41
%DIF < \usepackage[left=2.4cm,right=2.4cm,top=2.5cm,bottom=2.5cm]{geometry}
%DIF < %\usepackage[left=1.5in,right=1.5in,top=1.5in,bottom=1.5in]{geometry} % AER
%DIF < \linespread{1.5}
%DIF -------
%\usepackage[left=2.4cm,right=2.4cm,top=2.5cm,bottom=2.5cm]{geometry} %DIF > 
...
%DIF 47-48c53-58
%DIF < % TODO: check corrections sur mon .pdf (adrien)
...
}\DIFaddend \parbox{15cm}{\noindent {\small Using a representative survey, we find that after the Yellow Vests movement, French people would largely reject a Tax \& Dividend policy, i.e. a carbon tax whose revenues are redistributed uniformly to each adult. However, they overestimate their net monetary loss, wrongly think the policy is regressive, and do not perceive it as environmentally effective.  \DIFdelbegin \textit{\DIFdel{Correcting these three biases would suffice to generate majority approval. }}%DIFAUXCMD
\DIFdelend \DIFaddbegin \textit{\DIFadd{We predict a widespread approval if these three biases could be corrected.}}\DIFaddend  Yet, only a small minority can be convinced. Indeed, if overly pessimistic beliefs cause tax rejection, they also result from it through motivated reasoning, which manifests what we define as ``tax aversion''.}}  %DIF <  TODO: to all households -> to each adult; et ailleurs dans le papier aussi
%DIF <  TODO: #3 "correcting these three biases" not caveated -> replace by "if we could correct these three biases"?
...
%DIFDELCMD < %%%
\DIFdelend % DONE: one's beliefs about winning or losing from the policy => one's beliefs about winning or losing purchasing power from the policy
...
%DIF <  TODO: change positive to ego-positive (same thing for negative) for Eil & Rao (also, there is mixed evidence on this result, see p. 6 of Thaler 2019 for references)
%DIF <  TODO: cite Thaler (2019) as example of MR with political attitudes
...
%DIF <  TODO: justifier que c'est pas incentivized (pck dans la vraie vie quand les gens forment leurs opinions politiques, ils sont pas incentivized). les incitations sur leurs gains ça aurait compliqué l'histoire, parce que les gens auraient pas cherché à donner leur vrai gain, mais ce qu'ils pensent que l'on pense être leur vrai gain, ce qui n'est pas forcément la même chose
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{figure}[!htbp]
\DIFaddendFL \makebox[\textwidth][c]{
...
\end{tabular} 
} {\footnotesize \\ \quad \\ \textsc{Note:} Standard errors are reported in parentheses. For logit, average marginal effects are reported and not coefficients. The list of controls can be found in Appendix \ref{set_controls}.}  \end{table} %    \subsubsection{Subjective vs. objective elasticities}
...
%DIF <  TODO: Erreur dans Alterea. Faire le calcul nous-mêmes à partir de CGDD (2018): p. 20, on voit que émissions baissent de 450 Mt en 2015 à 85 Mt en 2050, soit -4.6% / an pendant 35 ans.
...
\hline 
\hline \\[-1.8ex] 
\DIFdelbeginFL %DIFDELCMD < & \multicolumn{4}{r}{p$<$0.1; p$<$0.05; p$<$0.01} \\ 
%DIFDELCMD < %%%
...
% \\[-1.8ex] & (1) & (2) & (3)\\ 
% \hline \\[-1.8ex] 
%DIF <   Constant & 0.120 & $-$0.041 & $-$0.150 \\ 
%DIF >   Constant & 0.120 & $-$0.041 & $-$0.150 \\ 
%   & (0.012) & (0.190) & (0.189) \\ 
%DIF <    Winner, before feedback ($\dot{G}$) & 0.695 & 0.685 & 0.646 \\ 
%DIF >    Winner, before feedback ($\dot{G}$) & 0.695 & 0.685 & 0.646 \\ 
%   & (0.078) & (0.080) & (0.080) \\ 
%DIF <    Initial tax: PNR (I don't know) &  &  & 0.163 \\ 
%DIF >    Initial tax: PNR (I don't know) &  &  & 0.163 \\ 
%   &  &  & (0.031) \\ 
%DIF <    Initial tax: Approves &  &  & 0.158 \\ 
%DIF >    Initial tax: Approves &  &  & 0.158 \\ 
%   &  &  & (0.046) \\ 
%DIF <    Retired &  & 0.143 & 0.146 \\ 
%DIF >    Retired &  & 0.143 & 0.146 \\ 
%   &  & (0.080) & (0.079) \\ 
%DIF <    Active &  & 0.165 & 0.175 \\ 
%DIF >    Active &  & 0.165 & 0.175 \\ 
%   &  & (0.055) & (0.054) \\ 
%DIF <    Student &  & 0.249 & 0.234 \\ 
%DIF >    Student &  & 0.249 & 0.234 \\ 
%   &  & (0.076) & (0.075) \\ 
%   Yellow Vests: PNR &  & $-$0.048 & $-$0.043 \\ 
%   &  & (0.047) & (0.047) \\ 
%DIF <    Yellow Vests: understands &  & $-$0.090 & $-$0.063 \\ 
%DIF >    Yellow Vests: understands &  & $-$0.090 & $-$0.063 \\ 
%   &  & (0.034) & (0.034) \\ 
%DIF <    Yellow Vests: supports &  & $-$0.101 & $-$0.059 \\ 
%DIF >    Yellow Vests: supports &  & $-$0.101 & $-$0.059 \\ 
%   &  & (0.035) & (0.036) \\ 
%DIF <    Yellow Vests: is part of &  & $-$0.172 & $-$0.137 \\ 
%DIF >    Yellow Vests: is part of &  & $-$0.172 & $-$0.137 \\ 
%   &  & (0.062) & (0.062) \\ 
%  \hline \\[-1.8ex] 
...
\DIFdelbegin \DIFdel{While }\DIFdelend %DIF >  DONEw: replace "While three-quarters of the respondents are expected to win from our Tax \& Dividend, 62\% of these winners consider that they would not win and disapprove of the policy." by "Among the three-quarters of the respondents expected to win from our Tax \& Dividend, 62\% both consider that they would not win and disapprove of the policy."
...
\hline 
\hline \\[-1.8ex] 
  \DIFdelbeginFL %DIFDELCMD < & \multicolumn{6}{r}{p$<$0.1; p$<$0.05; p$<$0.01} \\ 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL 
...
\hline 
\hline \\[-1.8ex] 
\DIFdelbeginFL %DIFDELCMD < & \multicolumn{5}{r}{p$<$0.1; p$<$0.05; p$<$0.01} \\ 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL 
...
%DIF <  TODO: retirer "endogenously"? (pareil dans abstract)
...
\hline 
\hline \\[-1.8ex] 
\DIFdelbeginFL %DIFDELCMD < & \multicolumn{3}{r}{p$<$0.1; p$<$0.05; p$<$0.01} \\ 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL 
...
%DIF <  TODO ajouter un genre de "disclaimer" en footnote au début pour prévenir que notre intention n'est pas de pointer du doigt certaines catégories, mais de mettre en évidence des mécanismes cognitifs assez généraux au travers elles.
...
%DIF <  TODO: rajouter une note sur le fait que le gain moyen en pouvoir d'achat est positif
\begin{figure}[H]
\begin{subfigure}{.5\textwidth}
\centering
...
\DIFaddend \subsection{Progressivity \label{subsec:app-prog}}
%\vspace{-0.4cm}
...
%DIF <  DONE: removed " (column 3 of Table \ref{asymmetric_simple}, reproduced as column 1 in Table \ref{tab:robustness_mr})"
...
\hline 
\hline \\[-1.8ex] 
\DIFdelbeginFL %DIFDELCMD < & \multicolumn{2}{r}{p$<$0.1; p$<$0.05; p$<$0.01} \\ 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL 
...
% \\[-1.8ex] & (1) & (2) & (3) & (4) & (5) & (6)\\ 
% \hline \\[-1.8ex] 
%DIF <   Believes wins & 0.636 & 0.382 &  & 0.754 & 0.534 &  \\ 
%DIF >   Believes wins & 0.636 & 0.382 &  & 0.754 & 0.534 &  \\ 
%   & (0.100) & (0.080) &  & (0.197) & (0.163) &  \\ 
%DIF <    Believes does not lose &  &  & 0.351 &  &  & 0.349 \\ 
%DIF >    Believes does not lose &  &  & 0.351 &  &  & 0.349 \\ 
%   &  &  & (0.073) &  &  & (0.107) \\ 
%DIF <    Initial tax Acceptance ($A^I$) &  &  &  & 0.384 & 0.487 & 0.527 \\ 
%DIF >    Initial tax Acceptance ($A^I$) &  &  &  & 0.384 & 0.487 & 0.527 \\ 
%   &  &  &  & (0.071) & (0.059) & (0.049) \\ 
%  \hline \\[-1.8ex] 
...
% Lien questionnaire au cas où: https://hbs.qualtrics.com/jfe/form/SV_9zqdJWZXgpWfjsF
 ...
 %DIF <  TODO: rajouter le questionnaire, cf. ci-dessous
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \paragraph{Priming}
%DIF <  \begin{enumerate}
%DIF <  \item {[}No priming{]} Welcome to this survey. \\
%DIF <  It was conceived by two researchers in social science. It lasts about
%DIF <  15-20 minutes.
%DIF <  \item {[}Info PM{]} Welcome to this survey. \\
%DIF <  It was conceived by two researchers in social science. It lasts about
%DIF <  15-20 minutes.\\
%DIF <  \\
%DIF <  Before starting, please read carefully the information below on particulate
%DIF <  matter pollution: 
%DIF <  \end{enumerate}
%DIF <  \begin{itemize}
%DIF <  \item particulate matter are responsible for 48,000 deaths in France each
%DIF <  year; 
%DIF <  \item particulate matter reduce the life expectancy of French people by
%DIF <  9 months; 
%DIF <  \item reducing fuel consumption would reduce the health problems associated
%DIF <  with particulate matter. \\
%DIF <  \\
%DIF <  Source: \href{http://invs.santepubliquefrance.fr/Publications-et-outils/Rapports-et-syntheses/Environnement-et-sante/2016/Impacts-de-l-exposition-chronique-aux-particules-fines-sur-la-mortalite-en-France-continentale-et-analyse-des-gains-en-sante-de-plusieurs-scenarios-de-reduction-de-la-pollution-atmospherique}{France Public Health Report (2016)}
%DIF <  \end{itemize}
%DIF <  \begin{enumerate}
%DIF <  \item {[}Info CC{]} Welcome to this survey. \\
%DIF <  It was conceived by two researchers in social science. It lasts about
%DIF <  15-20 minutes.\\
%DIF <  \\
%DIF <  Please read carefully the information below on climate change. \\
%DIF <  \end{enumerate}
%DIF <  \begin{itemize}
%DIF <  \item Climate change is already responsible for 150,000 deaths annually.
%DIF <  \item If greenhouse gas emissions continue on their current trend, the average
%DIF <  global warming will be +5°C in 2100 and +8°C in 2250. 
%DIF <  \item A rapid transition to renewable energies is technically possible and
%DIF <  would contain global warming at +2°C. 
%DIF <  \end{itemize}
%DIF <  According to scientists, in the absence of ambitious measures: 
%DIF <  \begin{itemize}
%DIF <  \item a large proportion of species face an increased risk of extinction;
%DIF <  \item natural disasters will intensify (hurricanes, heat waves, droughts,
%DIF <  floods, forest fires, etc.); 
%DIF <  \item by 2100, 270 million more people would be flooded each year due to
%DIF <  sea-level rise; 
%DIF <  \item violent conflicts and migration flows can be expected to increase. 
%DIF <  \end{itemize}
%DIF <  Sources: \href{http://www.pnas.org/content/106/49/20670}{Burke et al (2009)},
%DIF <  \href{http://www.pnas.org/content/pnas/early/2014/01/29/1222469111.full.pdf}{Hinkel et al (2014)},
%DIF <  \href{http://www.ipcc.ch/report/ar5/syr/}{IPCC Report (2014)}, \href{http://sci-hub.tw/10.1007/s10584-011-0156-z}{Meinshausen et al (2011)},
%DIF <  \href{http://sci-hub.tw/https\%3A/www.nature.com/articles/nature04188}{Patz et al (2005)}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \paragraph{Socio-demographics}
%DIF <  \begin{enumerate}
%DIF <  \item What is your postal code? 
%DIF <  \item What is your gender (in the sense of civil status)? \emph{}\\
%DIF <  \emph{Female; Male }
%DIF <  \item What is your age group? \emph{}\\
%DIF <  \emph{18 to 24 years old; 25 to 34 years old; 35 to 49 years old;
%DIF <  50 to 64 years old; 65 years old or more} 
%DIF <  \item What is your employment status? \emph{}\\
%DIF <  \emph{Permanent; Temporary contract; Unemployed; Student; Retired;
%DIF <  Other active; Inactive}
%DIF <  \item What is your socio-professional category? (Remember that the unemployed
%DIF <  are active workers). \emph{}\\
%DIF <  \emph{Farmer; Craftsperson, merchant; Independent; Executive; Intermediate
%DIF <  occupation; Employee; Worker; Retired; Other Inactive} 
%DIF <  \item What is your highest degree? \emph{}\\
%DIF <  \emph{No diploma; Brevet des collèges; CAP or BEP {[}secondary{]};
%DIF <  Baccalaureate; Bac +2 (BTS, DUT, DEUG, schools of health and social
%DIF <  training...); Bac +3 (licence...) {[}bachelor{]}; Bac +5 or more (master,
%DIF <  engineering or business school, doctorate, medicine, master, DEA,
%DIF <  DESS...)}
%DIF <  \item How many people live in your household? Household includes: you, your
%DIF <  family members who live with you, and your dependents. 
%DIF <  \item What is your net \textbf{\uline{monthly}} income (in euros)? \textbf{\uline{All
%DIF <  income}} (before withholding tax) is included here: salaries, pensions,
%DIF <  allowances, APL {[}housing allowance{]}, land income, etc. 
%DIF <  \item What is the net \textbf{\uline{monthly}} income (in euros) \textbf{\uline{of
%DIF <  your household}}? \textbf{\uline{All income}} (before withholding
%DIF <  tax) is included here: salaries, pensions, allowances, APL {[}housing
%DIF <  allowance{]}, land income, etc. 
%DIF <  \item In your household how many people are 14 years old or older (\textbf{\uline{including
%DIF <  yourself}})? 
%DIF <  \item In your household, how many people are over the age of majority (\textbf{\uline{including
%DIF <  yourself}})? 
%DIF <  \end{enumerate}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \paragraph{Energy characteristics}
%DIF <  \begin{enumerate}
%DIF <  \item What is the surface area of your home? (in m\texttwosuperior )
%DIF <  \item What is the heating system in your home? \emph{}\\
%DIF <  \emph{Individual heating; Collective heating; PNR (Don't know, don't
%DIF <  say)}
%DIF <  \item What is the main heating energy source in your home? \emph{}\\
%DIF <  \emph{Electricity Town gas; Butane, propane, tank gas; Heating oil;
%DIF <  Wood, solar, geothermal, aerothermal (heat pump); Other; PNR (Don't
%DIF <  know, don't say)}
%DIF <  \item How many motor vehicles does your household have? \emph{}\\
%DIF <  \emph{None; One; Two or more} 
%DIF <  \item {[}Without a vehicle{]} How many kilometers have you driven in the
%DIF <  last 12 months? 
%DIF <  \item {[}One vehicle{]} What type of fuel do you use for this vehicle? \emph{}\\
%DIF <  \emph{Electric or hybrid; Diesel; Gasoline; Other} 
%DIF <  \item {[}One vehicle{]} What is the average fuel economy of your vehicle?
%DIF <  (in Liters per 100 km)
%DIF <  \item {[}One vehicle{]} How many kilometers have you driven with your vehicle
%DIF <  in the last 12 months?
%DIF <  \item {[}At least two vehicles{]} What type of fuel do you use for your
%DIF <  main vehicle?\\
%DIF <   \emph{Electric or hybrid; Diesel; Gasoline; Other} 
%DIF <  \item {[}At least two vehicles{]} What type of fuel do you use for your
%DIF <  second vehicle?\\
%DIF <   \emph{Electric or hybrid; Diesel; Gasoline; Other} 
%DIF <  \item {[}At least two vehicles{]} What is the average fuel economy of all
%DIF <  your vehicles? (in Liters per 100 km) 
%DIF <  \item {[}At least two vehicles{]} How many kilometers have you driven with
%DIF <  all your vehicles in the last 12 months? 
%DIF <  \end{enumerate}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \paragraph{Partial reforms {[}transport / housing{]}}
%DIF <  \begin{enumerate}
%DIF <  \item Do you think that an increase in VAT would result in a loss of more
%DIF <  purchasing power for your household than for the average French household?
%DIF <  \emph{}\\
%DIF <  \emph{Yes, much more; Yes, a little more; As much as the average;
%DIF <  No, a little less; No, a lot less; PNR (Don't know, don't say)} 
%DIF <  \item Do you think that an increase in {[}fuel taxes / taxes on gas and
%DIF <  heating oil{]} would cause your household to lose more purchasing
%DIF <  power than an average French household? \emph{}\\
%DIF <  \emph{Yes, much more; Yes, a little more; As much as the average;
%DIF <  No, a little less; No, a lot less; PNR (Don't know, don't say)}
%DIF <  \item The government is studying a fuel tax increase, whose revenues would
%DIF <  be redistributed to all households, regardless of their income. This
%DIF <  would imply: 
%DIF <  \end{enumerate}
%DIF <  \begin{itemize}
%DIF <  \item {[}an increase in the price of gasoline by 11 cents per liter and
%DIF <  diesel by 13 cents per liter / a 13\% increase in the price of gas,
%DIF <  and a 15\% increase in the price of heating oil{]}; 
%DIF <  \item an annual payment of {[}60 / 50{]}¤ to each adult, or {[}120 / 100{]}¤
%DIF <  per year for a couple. \\
%DIF <  \\
%DIF <  \textbf{In terms of purchasing power, would your household be a winner
%DIF <  or a loser with such a measure?} \emph{}\\
%DIF <  \emph{Winner; Unaffected; Loser} 
%DIF <  \end{itemize}
%DIF <  \begin{enumerate}
%DIF <  \item {[}\emph{Winner} selected{]} \textbf{According to you, your household's
%DIF <  purchasing power would increase:} \emph{}\\
%DIF <  \emph{From 0 to {[}10$\cdot$uc{]} ¤ per year; From {[}10$\cdot$uc{]}
%DIF <  to {[}20$\cdot$uc{]} ¤ per year; From {[}20$\cdot$uc{]} to {[}30$\cdot$uc{]}
%DIF <  ¤ per year; From {[}30$\cdot$uc{]} to {[}40$\cdot$uc{]} ¤ per year;
%DIF <  More than {[}40$\cdot$uc{]} ¤ per year}
%DIF <  \item {[}\emph{Loser} selected{]} \textbf{According to you, the purchasing
%DIF <  power of your household would decrease:} \emph{}\\
%DIF <  \emph{From 0 to {[}15$\cdot$uc{]} ¤ per year; From {[}15$\cdot$uc{]}
%DIF <  to {[}40$\cdot$uc{]} ¤ per year; From {[}40$\cdot$uc{]} to {[}70$\cdot$uc{]}
%DIF <  ¤ per year; From {[}70$\cdot$uc{]} to {[}110$\cdot$uc{]} ¤ per year;
%DIF <  From {[}110$\cdot$uc{]} to {[}160$\cdot$uc{]} ¤ per year; From more
%DIF <  than {[}160$\cdot$uc{]} ¤ per year}
%DIF <  \item If fuel prices increased by 50 cents per liter, by how much would
%DIF <  \textbf{\uline{your household}} reduce its fuel consumption? \emph{}\\
%DIF <  \emph{0\% -} {[}\emph{I already consume almost none }/\emph{ I am
%DIF <  already not consuming}{]}\emph{; 0\% - }{[}\emph{I am constrained
%DIF <  on all my trips} / \emph{I will not reduce it}{]}\emph{; From 0\%
%DIF <  to 10\%; From 10\% to 20\%; From 20\% to 30\%; More than 30\% - }{[}\emph{I
%DIF <  would change my travel habits significantly }/ \emph{I would change
%DIF <  my consumption significantly}{]}
%DIF <  \item In your opinion, if {[}fuel prices increased by 50 cents per liter
%DIF <  / gas and heating oil prices increased by 30\%{]}, by how much would
%DIF <  \textbf{\uline{French people}} reduce their consumption on average?
%DIF <  \emph{}\\
%DIF <  \emph{From 0\% to 3\%; From 3\% to 10\%; From 3\% to 10\%; From 10\%
%DIF <  to 20\%; From 20\% to 30\%; More than 30\%} 
%DIF <  \end{enumerate}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \paragraph{Tax \& dividend: initial}
%DIF <  \begin{enumerate}
%DIF <  \item The government is studying an increase in the carbon tax, whose revenues
%DIF <  would be redistributed to all households, regardless of their income.
%DIF <  This would imply: 
%DIF <  \end{enumerate}
%DIF <  \begin{itemize}
%DIF <  \item an increase in the price of gasoline by 11 cents per liter and diesel
%DIF <  by 13 cents per liter; 
%DIF <  \item an increase of 13\% in the price of gas, and 15\% in the price of
%DIF <  heating oil;
%DIF <  \item an annual payment of 110¤ to each adult, or 220¤ per year for a couple.
%DIF <  \\
%DIF <  \\
%DIF <  \textbf{In terms of purchasing power, would your household win or
%DIF <  loser with such a measure? }\emph{}\\
%DIF <  \emph{Win; Be unaffected; Lose}
%DIF <  \end{itemize}
%DIF <  \begin{enumerate}
%DIF <  \item {[}\emph{Winner} selected{]} \textbf{According to you, your household's
%DIF <  purchasing power would increase:} \emph{}\\
%DIF <  \emph{From 0 to {[}20$\cdot$uc{]} ¤ per year; From {[}20$\cdot$uc{]}
%DIF <  to {[}40$\cdot$uc{]} ¤ per year; From {[}40$\cdot$uc{]} to {[}60$\cdot$uc{]}
%DIF <  ¤ per year; From {[}60$\cdot$uc{]} to {[}80$\cdot$uc{]} ¤ per year;
%DIF <  From more than {[}80$\cdot$uc{]} ¤ per year}
%DIF <  \item {[}\emph{Loser} selected{]} \textbf{According to you, the purchasing
%DIF <  power of your household would decrease:} \emph{}\\
%DIF <  \emph{From 0 to {[}30$\cdot$uc{]} ¤ per year; From {[}30$\cdot$uc{]}
%DIF <  to {[}70$\cdot$uc{]} ¤ per year; From {[}70$\cdot$uc{]} to {[}120$\cdot$uc{]}
%DIF <  ¤ per year; From {[}120$\cdot$uc{]} to {[}190$\cdot$uc{]} ¤ per
%DIF <  year; From {[}190$\cdot$uc{]} to {[}280$\cdot$uc{]} ¤ per year;
%DIF <  From more than {[}280$\cdot$uc{]} ¤ per year} 
%DIF <  \item {[} {[}empty{]} / Scientists agree that a carbon tax would be effective
%DIF <  in reducing pollution.{]} Do you think that such a measure would reduce
%DIF <  pollution and fight climate change? \emph{}\\
%DIF <  \emph{Yes; No; PNR (Don't know, don't say)}
%DIF <  \item In your opinion, which categories would lose {[} {[}blank{]} / purchasing
%DIF <  power{]} with such a measure? (Several answers possible) \emph{}\\
%DIF <  \emph{No one; The poorest; The middle classes; The richest; All French
%DIF <  people; Rural or peri-urban people; Some French people, but not a
%DIF <  particular income category; PNR (Don't know, don't say)} 
%DIF <  \item In your opinion, what categories would gain purchasing power with
%DIF <  such a measure? (Several answers possible) \emph{}\\
%DIF <  \emph{No one; The poorest; The middle classes; The richest; All French
%DIF <  people; Urban dwellers; Some French people, but not a particular income
%DIF <  category; PNR (Don't know, don't say)} 
%DIF <  \end{enumerate}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \paragraph{Tax \& dividend: after knowledge}
%DIF <  \begin{enumerate}
%DIF <  \item {[}Feedback{]} We always consider the same measure. As a reminder,
%DIF <  it would imply: 
%DIF <  \end{enumerate}
%DIF <  \begin{itemize}
%DIF <  \item an increase in the price of petrol by 11 cents per liter and diesel
%DIF <  by 13 cents per liter; 
%DIF <  \item an increase of 13\% in the price of gas, and 15\% in the price of
%DIF <  heating oil; 
%DIF <  \item an annual payment of 110¤ to each adult, or 220¤ per year for a couple. 
%DIF <  \end{itemize}
%DIF <  In five out of six cases, a household with the same characteristics
%DIF <  as yours would \textbf{\uline{{[}win / lose{]}}}. (The characteristics
%DIF <  taken into account are: heating with {[}source{]} for a dwelling of
%DIF <  {[}size{]} m\texttwosuperior ; {[}distance{]} km covered with an average
%DIF <  consumption of {[}fuel economy{]} liters per 100 km). \\
%DIF <  \\
%DIF <  Based on this estimate, do you now think that your household would
%DIF <  be: \emph{}\\
%DIF <  \emph{Winner; Unaffected; Loser} 
%DIF <  \begin{enumerate}
%DIF <  \item {[}Info on progressivity{]} On average, this measure would increase
%DIF <  the purchasing power of poorest households, and decrease that of the
%DIF <  richest, who consume more energy. \\
%DIF <  \\
%DIF <  In view of this new information, do you think this measure would benefit
%DIF <  the poorest? \emph{}\\
%DIF <  \emph{Yes; No; PNR (Don't know, don't say)}
%DIF <  \item {[}No info on progressivity{]} Do you think this measure would benefit
%DIF <  the poorest?\\
%DIF <  \emph{Yes; No; PNR (Don't know, don't say)}
%DIF <  \item In view of the above estimate, would you approve of such a measure?
%DIF <  \emph{}\\
%DIF <  \emph{Yes; No; PNR (Don't know, don't say)} 
%DIF <  \item Why do you think this measure is beneficial? (Maximum three responses)
%DIF <  \emph{}\\
%DIF <  \emph{Contributes to the fight climate change; Reduces the harmful
%DIF <  effects of pollution on health; Reduces traffic congestion; Increases
%DIF <  my purchasing power; Increases the purchasing power of the poorest;
%DIF <  Fosters France's independence from fossil energy imports; Prepares
%DIF <  the economy for tomorrow's challenges; For none of these reasons;
%DIF <  Other (specify): }
%DIF <  \item Why do you think this measure is unwanted? (Maximum three answers)
%DIF <  \emph{}\\
%DIF <  \emph{Is ineffective in reducing pollution; Alternatives are insufficient
%DIF <  or too expensive; Penalizes rural areas; Decreases my purchasing power;
%DIF <  Decreases the purchasing power of some modest households; Harms the
%DIF <  economy and employment; Is a pretext for raising taxes; For none of
%DIF <  these reasons; Other (specify):} 
%DIF <  \end{enumerate}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \paragraph{Tax \& targeted dividend}
%DIF <  \begin{enumerate}
%DIF <  \item The government is studying an increase in the carbon tax, whose revenues
%DIF <  would be redistributed \textbf{\uline{to the {[}20 / 30 / 40 /
%DIF <  50{]}\% of the poorest French people only}}. This would imply: 
%DIF <  \end{enumerate}
%DIF <  \begin{itemize}
%DIF <  \item an increase in the price of gasoline by 11 cents per liter and diesel
%DIF <  by 13 cents per liter; 
%DIF <  \item an increase of 13\% in the price of gas, and 15\% in the price of
%DIF <  heating oil; 
%DIF <  \item an annual payment of {[}550 / 360 / 270 / 220{]}¤ for each adult earning
%DIF <  less than {[}780 / 1140 / 1430 / 1670{]}¤ per month (welfare benefits
%DIF <  included, before withholding tax); 
%DIF <  \item no compensation for the others. 
%DIF <  \end{itemize}
%DIF <  We estimate that in your household, {[}number of recipients{]} persons
%DIF <  would receive this payment. \\
%DIF <  \\
%DIF <  In terms of purchasing power, would your household win or lose with
%DIF <  such a measure? \emph{}\\
%DIF <  \emph{Win; Be unaffected; Lose}
%DIF <  \begin{enumerate}
%DIF <  \item Would you approve such a measure? \emph{}\\
%DIF <  \emph{Yes; No; PNR (Don't know, don't say)} 
%DIF <  \end{enumerate}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \paragraph{Other questions}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  The survey is completed by other attitudinal questions, treated in
%DIF <  our companion paper, TODO: cite. Hereafter, we only describe questions
%DIF <  that are used in the present paper.
%DIF <  \begin{enumerate}
%DIF <  \item Please select ``A little'' (test to check that you are attentive).
%DIF <  \emph{}\\
%DIF <  \emph{Not at all; A little; A lot; Completely; PNR (Don't know, don't
%DIF <  say)}
%DIF <  \item Do you smoke regularly? \emph{Yes; No}
%DIF <  \item How much are you interested in politics? \emph{}\\
%DIF <  \emph{Almost not; A little; A lot }
%DIF <  \item How would you define yourself? (Several answers possible) \emph{}\\
%DIF <  \emph{Extreme left; Left; Center; Right; Extreme right; Liberal; Conservative;
%DIF <  Liberal; Humanist; Patriot; Apolitical; Ecologist }
%DIF <  \item How do you keep yourself informed of current events? Mainly through...
%DIF <  \emph{}\\
%DIF <  \emph{Television; Press (written or online); Social networks; Radio;
%DIF <  Other}
%DIF <  \item What do you think of the Yellow Vests? (Several answers possible)
%DIF <  \emph{}\\
%DIF <  \emph{I am part of them; I support them; I understand them; I oppose
%DIF <  them; PNR (Don't know, don't say)}
%DIF <  \item The survey is nearing completion. You can now enter any comments,
%DIF <  comments or suggestions in the field below.
%DIF <  \end{enumerate}
