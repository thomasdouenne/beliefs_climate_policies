...
\[
\mathbb{P}_{i,t}\left(G_{i}>0\right)=f\left(\underset{+}{g_{i}}\right)
\]
et on estime \emph{f}. On a le gain subjectif $g_{i}=\gamma_{i}-b+\epsilon_{i}$
...
\begin{align*}
g_{i} & =\underset{\gamma_{i}}{\underbrace{110-\Delta E_{i}}}-b+\epsilon_{i}\\
 & =\underset{\widehat{\gamma_{i}}}{\underbrace{110-\Delta\widehat{E_{i}}}}+\iota_{i}-b+\epsilon_{i}
\end{align*}

Le répondant sait qu'il commet une erreur $b-\epsilon_{i}$, qu'il
croit d'espérance nulle (il pense que $b=0$ \emph{a priori}). Il
croit que nous estimons ses hausses de dépenses à $\widehat{\gamma_{i}}-\epsilon_{i}=\gamma_{i}-\iota_{i}-\epsilon_{i}$,
et que nous commettons une erreur d'espérance $b-\iota_{i}$ \emph{a
priori }non nulle, où $\iota_{i}-b$ est son information partielle
(liée à ses caractéristiques inobservées par nous). On peut décomposer
$b-\iota_{i}$ entre son biais $b$ et le nôtre $\iota_{i}$ (qu'on
peut estimer dans une extension pour estimer \emph{b} plus précisément).
Le répondant sait qu'il peut être biaisé. Il sait que l'écart $g_{i}-\widehat{\gamma_{i}}$
entre son estimation et la nôtre est de $\iota_{i}-b+\epsilon_{i}$,
mais ne sait pas quel part de cet écart est dû à son information partielle
$\iota_{i}-b$, et quelle part est due à son incertitude $\epsilon_{i}$.
L'incertitude du répondant $\epsilon_{i}$ est une variable aléatoire
centrée qui l'empêche de distinguer son biais \emph{b} du nôtre $\iota_{i}$.
...
\[
g_{i}^{F}=\widehat{\gamma_{i}}+\iota_{i}-\left(\alpha+\eta_{i}\right)b+\epsilon_{i}
\]
...
\[
\widehat{g_{i}^{F}}=\hat{f}^{-1}\left(\mathbb{P}_{i,t+1}\left(G_{i}^{F}>0\right)\right)
\]
...
$g_{i}-g_{i}^{F}=b\left(\alpha+\eta_{i}-1\right)$:

\begin{align*}
\widehat{\alpha} & =1+\frac{g_{i}-\widehat{g_{i}^{F}}}{b}
\end{align*}

TODO: mettre ça dans un cadre bayésien (le 5/6 n'intervient pas par
exemple!). Pistes : $\widetilde{\mathbb{P}}\left(\Gamma>0\right)\overset{\text{hyp.}}{=}\widetilde{\mathbb{P}}\left(\widehat{\Gamma}>0\right)\implies\widetilde{\mathbb{P}}\left(\widehat{\Gamma}>0\,|\,\Gamma>0\right)=\widetilde{\mathbb{P}}\left(\Gamma>0\,|\,\widehat{\Gamma}>0\right)=\frac{5}{6}$;
